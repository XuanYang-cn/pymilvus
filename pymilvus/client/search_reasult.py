...
class SearchResult(list):
    ...
    def __str__(self) -> str:
        ...
        result_msg = f"data: {list(map(str, self[:10]))}"

        # Optional messages
        recall_msg = f",recalls: {list(map(str, self.recalls[:10]))}" if self.recalls else ""
    ...
    def __init__(
        ...
        for i in range(topk):
            ...
            for fname, (data, field_meta) in fields.items():
                ...
                entity[fname] = data[i]
            top_k_res.append({pk_name: pks[i], "distance": distances[i], "entity": entity})
